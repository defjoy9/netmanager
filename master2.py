    ...
    try:
        ...
        try:
            ...
            for item in device_info:
                ...
                if delete_files(local_export_file) == 0:
                    msg = f"Problem occured while trying to delete {local_export_file} locally"
                    logging.error(msg)
                    local_del_fail += 1
                ...
                if delete_files(local_backup_file) == 0:
                    msg = f"Problem occured while trying to delete {local_backup_file} locally:"
                    logging.error(msg)
                    local_del_fail += 1
                    device_report["actions"]["Deleting files locally"].append({
                        "comannd": "",
